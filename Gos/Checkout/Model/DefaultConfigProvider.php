...
class DefaultConfigProvider extends \Magento\Checkout\Model\DefaultConfigProvider
{
    ...
    private function getQuoteItemData()
    {
        ...
        if ($quoteId) {
            ...
            foreach ($quoteItems as $index => $quoteItem) {
                ...
                $quoteItemData[$index]['image_url'] = $this->imageHelper->init(
                    $quoteItem->getProduct(),
                    'category_page_grid'
                )->getUrl();
                // Get product_id
                ...
                // [1]: Simple product
                // [2]: Accessories
                if (isset($quoteItem->getOptions()[1])) {
                    $product_id = $quoteItem->getOptions()[1]->getData('product_id');
                    ...
                    $quoteItemData[$index]['holden_colour'] = $this->getOptionText('holden_colour', $product);
                    $quoteItemData[$index]['wheel_type'] = $product->getData('wheel_type');
                    $quoteItemData[$index]['interior'] = $product->getData('interior');
                    $quoteItemData[$index]['simple_image'] = $media_path.'catalog/product'.$product->getData('image');
                    $quoteItemData[$index]['holden_engine_and_transmission'] = $product->getData('holden_engine_and_transmission');
                    $quoteItemData[$index]['day_available'] = $product->getData('day_available');
                }
            }
        }
    ...
    public function getOptionText($attr, $product) {
        $optionId = $product->getData($attr);
        $attr = $product->getResource()->getAttribute($attr);
