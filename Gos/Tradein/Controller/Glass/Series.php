...
class Series extends \Magento\Framework\App\Action\Action
{
    /**
     * Glass Factory
     * 
    ...
    public function execute()
    {
        ...
        $variantCollection->addFieldToSelect('glass_series');
        ...
        $variantCollection->getSelect()->group('glass_series')->order('glass_series ASC');
        //print_r($years);
		...
		if ($variantCollection) {
       		...
       		foreach ($variantCollection->getData() as $variant) {
                //print_r($year);
                $variants[]['glass_series'] = trim($variant['glass_series']);
        	}
