...
class Variant extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $variantCollection->addFieldToSelect('glass_variant');
        ...
        $variantCollection->getSelect()->group('glass_variant')->order('glass_variant ASC');
        //print_r($years);
		...
		if ($variantCollection) {
       		...
       		foreach ($variantCollection->getData() as $variant) {
                //print_r($year);
                $variants[]['glass_variant'] = trim($variant['glass_variant']);
        	}
