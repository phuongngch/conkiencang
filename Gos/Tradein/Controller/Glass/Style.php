...
class Style extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $variantCollection->addFieldToSelect('glass_style');
        ...
        $variantCollection->getSelect()->group('glass_style')->order('glass_style ASC');
        //print_r($years);
		...
		if ($variantCollection) {
       		...
       		foreach ($variantCollection->getData() as $variant) {
                //print_r($year);
                $variants[]['glass_style'] = trim($variant['glass_style']);
        	}
