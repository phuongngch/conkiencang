...
class Size extends \Magento\Framework\App\Action\Action
{
    /**
     * Glass Factory
     * 
    ...
    public function execute()
    {
        ...
        $variantCollection->addFieldToSelect('glass_size');
        ...
        $variantCollection->getSelect()->group('glass_size')->order('glass_size ASC');
        //print_r($years);
		...
		if ($variantCollection) {
       		...
       		foreach ($variantCollection->getData() as $variant) {
                //print_r($year);
                $variants[]['glass_size'] = trim($variant['glass_size']);
        	}
